...
@foreach ($wroffice as $offwr)
<div class="row">

  <div class="col s9">
<video poster="{{ $offwr->foto_video }}" id="player" playsinline controls>
    <source src="{{ url($offwr->video) }}" type="video/mp4">
</video>

  </div>
  <div class="right col s3 scrollbar scrollbar-default" style="height: 59vh;">
    <div class="force-overflow">
@foreach ($wrmenu as $menu)

    <ul class="collection">
      <a href="{{ $wroffice->url($menu->id) }}">
...
@endforeach
  ...
  <div class="col s3 container">
    <ul class="pagination">
        <li class="waves-effect"><a href="{{ $wroffice->previousPageUrl() }}" class="anterior" title="Video anterior"><i class="material-icons">skip_previous</i></a></li>
        <li class="waves-effect"><a href="{{ $wroffice->url($wroffice->currentPage()) }}">{{ $wroffice->currentPage() }}</a></li>
        <li class="waves-effect"><a href="{{ $wroffice->nextPageUrl() }}" class="siguiente" title="Siguiente video"><i class="material-icons">skip_next</i></a></li>
      </ul>
  </div>
</div>
